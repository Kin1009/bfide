...
win.iconbitmap("C:/Program Files/bfide/icon.ico")
win.resizable(0, 0)
if not exists(f"C:/Users/{getuser()}/AppData/Local/bfide/settings.txt"):
    mkdir(f"C:/Users/{getuser()}/AppData/Local/bfide")
    with open(f"C:/Users/{getuser()}/AppData/Local/bfide/settings.txt", "w") as f:
        f.write("10")
def print_(text):
...
def help(f=None):
    w = Toplevel(win)
    w.withdraw()
    w.iconbitmap("C:/Program Files/bfide/icon.ico")
    w.title("Help")
    w.resizable(0, 0)
    Label(w, text="""Help:
1. Brainfuck
...
def stop(a=None):
   global run
   run = 0
mode = open(f"C:/Users/{getuser()}/AppData/Local/bfide/settings.txt", "r").read()
