...
win.resizable(0, 0)
if not exists(f"C:/Program Files/bfide/settings.txt"):
    with open(f"C:/Program Files/bfide/settings.txt", "w") as f:
        f.write("10")
def print_(text):
...
def stop(a=None):
   global run
   run = 0
mode = open(f"C:/Program Files/bfide/settings.txt", "r").read()
